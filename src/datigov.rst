...
E' possibile scaricare i metadati conformi al profilo DCAT-AP_IT per questa basi di dati in `RDF/Turtle <Metadati/metadatiDCATAPIT_datigov.ttl>`__, `RDF/XML <Metadati/metadatiDCATAPIT_datigov.rdf>`__ e `JSON-LD <Metadati/metadatiDCATAPIT_datigov.jsonld>`__. (dati.gov.it è descritto come il dataset di tutti i dataset aperti. Inoltre, essendo questa una fotografia di quanto disponibile alla data di pubblicazione delle presenti schede, la data di ultimo aggiornamento indicata nei file è quella della pubblicazione stessa delle schede).
